...
export function VoiceEnabledGuide() {
  ...
  const welcomeMessages = [
    userName
      ? `Hello ${userName}! I'm NOVA, your interactive AI guide.`
  ...
  const responses = {
    ...
    "what's your name": "My name is NOVA. I'm your AI guide to the TechNova technical society portal.",
    "who are you":
      "I'm NOVA, your AI assistant for the TechNova portal. I can help you navigate the site and answer questions about our tech society.",
    "my name is": "Nice to meet you! I'll remember your name for our conversation.",
  }

  useEffect(() => {
    // Try to get user name from localStorage
    const storedName = localStorage.getItem("techNovaUserName")
    if (storedName) {
        ...
        setMessages([
          {
            ...
            role: "assistant",
            content: userName
              ? `Hi ${userName}! I'm NOVA, your interactive guide to TechNova. How can I help you today?`
              : "Hi there! I'm NOVA, your interactive guide to TechNova. How can I help you today?",
            isNew: true,
          },
        ])
      }
  ...
  const handleSendMessage = (e?: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      if (
        normalizedInput.includes("my name is") ||
        normalizedInput.includes("i am ") ||
        normalizedInput.includes("i'm ")
      ) {
        const nameMatch = normalizedInput.match(/my name is (.+)|i am (.+)|i'm (.+)/)
        if (nameMatch) {
          const extractedName = (nameMatch[1] || nameMatch[2] || nameMatch[3]).trim().split(" ")[0]
          if (extractedName) {
            setUserName(extractedName)
            localStorage.setItem("techNovaUserName", extractedName)
            responseContent = `Nice to meet you, ${extractedName}! I'll remember your name. How can I help you with TechNova today?`
          }
        }
      }
  ...
  // Animated avatar with speech indicator
  const AnimatedAvatar = ({ type, size, speaking }: { type: string; size: string; speaking: boolean }) => (
    <div className="relative">
      <Avatar type={type as any} size={size as any} animated={speaking} />
      {speaking && (
        <div className="absolute -bottom-1 -right-1 flex items-center justify-center">
          <div className="relative w-4 h-4 bg-cyan-900 rounded-full flex items-center justify-center">
            {[0, 1, 2].map((i) => (
              <motion.div
                key={i}
                className="absolute w-0.5 h-2 bg-cyan-400 rounded-full"
                animate={{
                  height: [1, 4, 1],
                  opacity: [0.5, 1, 0.5],
                }}
                ...
                style={{ transform: `rotate(${i * 30 - 30}deg)` }}
              />
            ))}
          </div>
        </div>
                ...
                <div className="flex items-center">
                  <AnimatedAvatar
                    ...
                    speaking={
                      ...
                      messages[messages.length - 1].isNew
                    }
                  />
                  <div className="ml-3">
                  ...
                  <div className="flex-1 overflow-y-auto p-4 bg-black/30 rounded-md border border-primary/20 mb-4">
                    <div className="space-y-6">
                      <div className="flex items-start gap-3">
                        <AnimatedAvatar type="ai" size="md" speaking={false} />
                        <div className="bg-primary/10 rounded-lg p-4 max-w-[80%]">
                          <p className="text-primary/90">
                            {userName ? `Welcome back, ${userName}!` : "Welcome to TechNova!"} I'm NOVA, your AI guide
                            to the IITM BS Technical Society. I can help you navigate our portal and answer questions
                            about our events, tech domains, resources, and more.
                          </p>
                      ...
                      {messages.map((message) => (
                        ...
                        >
                          {message.role === "assistant" ? (
                            <AnimatedAvatar type="ai" size="md" speaking={audioEnabled && message.isNew} />
                          ) : (
                  ...
                  <form onSubmit={handleSendMessage} className="flex gap-2">
                    ...
                    {recognitionRef.current && (
                      <Button
                        ...
                        className={`${
                          isListening ? "bg-primary/40 hover:bg-primary/50" : "bg-primary/20 hover:bg-primary/30"
                        } text-primary`}
                      >
        ...
        {guideState === "tour" && (
          ...
          >
            <div className="w-full max-w-3xl cyber-card p-6 relative">
              <div className="flex justify-between items-start mb-4">
                <div className="flex items-center">
                  <AnimatedAvatar type="ai" size="md" speaking={audioEnabled && isTypingComplete} />
                  <div className="ml-3">
          ...
          >
            ...
            <div className="h-60 overflow-y-auto mb-3 p-3 bg-black/30 rounded-md border border-primary/20">
              {messages.map((message) => (
                ...
                >
                  {message.role === "assistant" ? (
                    <AnimatedAvatar type="ai" size="xs" speaking={audioEnabled && message.isNew} />
                  ) : (
                    <div className="w-6 h-6 rounded-full bg-secondary/20 flex items-center justify-center text-secondary">
                      <span className="text-[10px]">{userName?.charAt(0) || "Y"}</span>
                    </div>
                  )}
                  <div
